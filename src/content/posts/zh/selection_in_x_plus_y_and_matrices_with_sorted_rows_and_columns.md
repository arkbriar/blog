...
然而在翻阅览 Discuss 区的时候发现，这玩意居然有 O(n) (n 为行、列数) 的算法，来自一篇论文 [Selection in X + Y and Matrices with Sorted Rows and Columns](http://www.cse.yorku.ca/~andy/pubs/X+Y.pdf)，同时适用于另一道题 [Find k Pairs with Smallest Sums](https://leetcode.com/problems/find-k-pairs-with-smallest-sums/)，在此只做介绍，因为我不认为有人能在面试的时候写的出来...
