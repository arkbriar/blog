...
假设我们有节点i，那么可以通过以下过程获取所有兄弟节点（父节点为同一个节点）。
...
#### Parant in 2's Complement

不妨设这里是32位的环境，大家还记得负数 -i 的二进制表示么？对，就是 2^32 - i，所以我们有 i & -i = i & (2^32 - i)，这样我们可以从有符号数转换到无符号数的操作。

还记得上述获取兄弟节点的操作，i + (i & -i)，我们用 j = 2^32 - i 来重新组织一下这个式子：

i + (i & -i) = 2^32 - (j - (j & (2 ^32 - j))),

我们把它也映射成补码，2^32 - i - (i & -i) = j - (j & (2 ^32 - j))

十分熟悉了吧？就是j去掉最后一位1的操作，我们也可以把它看做一个找父节点的操作，公共祖先也是0。现在就可以清楚地看到了，其实 i + (i & -i) 的操作在补码中是一个对称的求父节点操作，原来的求父节点操作 i - (i & -i) 同理为兄弟节点的对称操作。
